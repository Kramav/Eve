...
|----------|-----------|--------|
| **★ TOP PRIORITY · Conversation Engine** | Rework conversation system → persistent, multi-turn, recovers from failure, no wake word between turns. Design approved (hybrid HSM + event intake + context stack + structured Outcome protocol). | ⏳ **Phase 1 BUILT 2026-07-12** (behind `conversation_engine` flag, off by default): [core/conversation.py](core/conversation.py) engine + `listener.listen_followup` no-wake window + main turn-loop; extension phrases, cancel, grace window, legacy pending_confirm/converse bridge. Tests [tests/test_conversation.py](tests/test_conversation.py). Design → [docs/CONVERSATION_ARCHITECTURE.md](docs/CONVERSATION_ARCHITECTURE.md). Phases 2–7 remain (Outcome migration, recovery, YouTube/Mode, slots, LLM-context). |
